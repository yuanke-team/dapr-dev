...
get_latest_release() {
    local res
    res=$(git ls-remote --refs --tags "https://mirror.ghproxy.com/https://github.com/$1.git" |
        cut -d '/' -f 3 |
        sort --version-sort |
        grep -e '^[0-9].[0-9].[0-9]$' |
        tail -1)
...
https://mirror.ghproxy.com/https://github.com/WasmEdge/WasmEdge/issues/new?assignees=&labels=&template=bug_report.md
${NC}
EOF
...
get_wasmedge_image_deps() {
    ...
    _downloader "https://mirror.ghproxy.com/https://github.com/second-state/WasmEdge-image/releases/download/$VERSION_IM_DEPS/WasmEdge-image-deps-$VERSION_IM_DEPS-$IM_DEPS_RELEASE_PKG"
...
install_wasmedge_image() {
    ...
    _downloader "https://mirror.ghproxy.com/https://github.com/second-state/WasmEdge-image/releases/download/$VERSION_IM/WasmEdge-image-$VERSION_IM-$RELEASE_PKG"
...
get_wasmedge_tensorflow_deps() {
    ...
    _downloader "https://mirror.ghproxy.com/https://github.com/second-state/WasmEdge-tensorflow-deps/releases/download/$VERSION_TF_DEPS/WasmEdge-tensorflow-deps-TF-$VERSION_TF_DEPS-$RELEASE_PKG"

    echo "Fetching WasmEdge-tensorflow-deps-TFLite-$VERSION_TF_DEPS"
    _downloader "https://mirror.ghproxy.com/https://github.com/second-state/WasmEdge-tensorflow-deps/releases/download/$VERSION_TF_DEPS/WasmEdge-tensorflow-deps-TFLite-$VERSION_TF_DEPS-$RELEASE_PKG"
...
install_wasmedge_tensorflow() {
    ...
    _downloader "https://mirror.ghproxy.com/https://github.com/second-state/WasmEdge-tensorflow/releases/download/$VERSION_TF/WasmEdge-tensorflow-$VERSION_TF-$RELEASE_PKG"

    echo "Fetching WasmEdge-tensorflowlite-$VERSION_TF"
    _downloader "https://mirror.ghproxy.com/https://github.com/second-state/WasmEdge-tensorflow/releases/download/$VERSION_TF/WasmEdge-tensorflowlite-$VERSION_TF-$RELEASE_PKG"

    echo "Fetching WasmEdge-tensorflow-tools-$VERSION_TF_TOOLS"
    _downloader "https://mirror.ghproxy.com/https://github.com/second-state/WasmEdge-tensorflow-tools/releases/download/$VERSION_TF_TOOLS/WasmEdge-tensorflow-tools-$VERSION_TF_TOOLS-$RELEASE_PKG"
...
main() {
    ...
    while getopts "e:hp:v:r:V-:" OPT; do
        # support long options: https://mirror.ghproxy.com/https://stackoverflow.com/a/28466267/519360
    ...
    if [ "$REMOVE_OLD" == "1" ] || [[ "$REMOVE_OLD" =~ ^([yY][eE][sS]|[yY])$ ]]; then
        if [ -f "$IPATH/env" ]; then
            bash <(wget -qO- https://mirror.ghproxy.com/https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/utils/uninstall.sh) -p "$IPATH" -q
        fi
    fi
